...
from daops_tester import settings
from daops_tester import path_utils
...
def build_dict(dr, proj_dict, base_dir):
    ...
    facets = dict([_ for _ in zip(comps, proj_dict['facet_rule'])])
 
    var_id = comps[proj_dict['var_index']]
    frequency = comps[proj_dict['freq_index']]
...
def write_all():
    ...
    for project in _get_project_list(): 
        ...
        d.update(settings.projects[project])
        
        start_dir = d['base_dir']
        if start_dir.startswith('/group_workspaces'):
            start_dir = os.path.join(start_dir, project)
